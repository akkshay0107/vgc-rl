...
from encoder import Encoder, ACT_SIZE, OBS_DIM
from RL import BattlePolicyNet
...
policy = BattlePolicyNet(obs_dim=OBS_DIM, act_size=ACT_SIZE).to(DEVICE)
...
for episode in range(1, NUM_EPISODES + 1):
    obs, _ = env.reset()
    traj = {
        agent_usernames[0]: {"obs": [], "masks": [], "actions": [], "log_probs": [], "values": [], "rewards": []},
        agent_usernames[1]: {"obs": [], "masks": [], "actions": [], "log_probs": [], "values": [], "rewards": []},
    }
    ...
    for step in range(MAX_STEPS_PER_EPISODE):
        ...
        for i, username in enumerate(agent_usernames):
            action_pair = np.array([int(sampled0[i].item()), int(sampled1[i].item())], dtype=np.int64)
    ...
    avg_return = (sum(traj[agent_usernames[0]]["rewards"]) + sum(traj[agent_usernames[1]]["rewards"])) / 2.0
    print(
