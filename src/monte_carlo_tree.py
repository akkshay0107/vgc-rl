...
class MCTNode:
    ...
    def best_child(self, c=math.sqrt(2)):
        ...
        for child in self.childs:
            if child.calc_UCB1(N, c) > bestUCB1:
                bestChild = child
                bestUCB1 = child.calc_UCB1(N, c)
        return bestChild
...
def mct_search(iters=500, root=MCTNode()):
...
def human_vs_bot_game():
    ...
    for turn in range(10):
        print(f"\n=== Turn {turn + 1}/10 ===")
        print(
            f"Current scores - Bot: {current_state.state[0][3]}, Player: {current_state.state[1][3]}"
        )
        print(
            f"Bot's remaining moves - Rock: {current_state.state[0][0]}, Paper: {current_state.state[0][1]}, Scissors: {current_state.state[0][2]}"
        )
        print(
            f"Your remaining moves - Rock: {current_state.state[1][0]}, Paper: {current_state.state[1][1]}, Scissors: {current_state.state[1][2]}"
        )
        ...
        if turn < 9:
            current_node = mct_search(500, current_node)
    ...
    while True:
        ...
        try:
            ...
            if player_resources[move] > 0:
                ...
                print("You must use your required moves first!")
